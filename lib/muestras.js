...
const { getNow, normalizeFechaCelda, normalizeHoraCelda } = require('./dateHelper');
...
function rowToMuestra(row) {
  const fecha = normalizeFechaCelda(row[1]);
  const hora = normalizeHoraCelda(row[2]);
...
async function deleteByCode(codigo) {
  try {
    ...
    for (let r = 0; r < data.length; r++) {
      const fecha = normalizeFechaCelda(data[r][1]);
...
async function deleteMuestraGrupo(codigo, fecha) {
  try {
    ...
    const filasABorrar = [];
    for (let r = 0; r < data.length; r++) {
      const fechaFila = normalizeFechaCelda(data[r][1]);
...
async function getMuro() {
  try {
    ...
    const mensajes = data.filter(r => r[0]).map(r => ({
      id: String(r[0]), fecha: normalizeFechaCelda(r[1]), hora: normalizeHoraCelda(r[2]),
